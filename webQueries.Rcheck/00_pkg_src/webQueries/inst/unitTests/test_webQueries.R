test_geneQueries <- function(){
    ...
    checkEquals(gettext(tmp <- runQuery(2064, "gene", bySymbol=FALSE)$Name),
        "ERBB2")
}
test_protQueries <- function(){
    checkEquals(gettext(tmp <- runQuery("erbb2", "protein")$Caption[1]),
        "P04626")
    checkEquals(gettext(tmp <- runQuery("P04626", "protein")$Caption[1]),
        "P04626")
}
...
test_multi <- function(){
    checkTrue(nrow(tmp <- multiQueries(c("egfr", "erbb2", "fgfr1"), "gene"))>0)	
}