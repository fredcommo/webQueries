...
# MAIN FUNCTIONS
geneQuery <- function(geneId, DB = "gene", bySymb = TRUE, kTries = 10, verbose = TRUE){
  ow <- options("warn")
  options(warn = -1)
...
#  ids <- geneId
  ...
  if(bySymb){
    ...
    ids <- unlist(.gsearch(paste0(query, "%5Bsymbol%5D%20homo%20sapiens"), DB, kTries))
		if(is.null(ids)){
      if(verbose){
...
protQuery <- function(protId, DB="protein", kTries=10, verbose=TRUE){
  ...
  if(is.null(ids)){
    ...
    return(NULL)
      ...
      return(
        c(query=protId, .getProtValues(gsummary))
      )
  }
}
...
.gsearch <- function (query, DB, kTries){
  ...
  # query : use official symbolss. Multiple requests are accepted, e.g. "EGFR, Homo sapiens"
...
.recName <- function(recname){
  ...
  if(any(grepl("RecName", recsplit)))
    rec <- gsub("(RecName: Full=)(.*)", "\\2", recsplit[grep("RecName", recsplit)])
  if(any(grepl("AltName", recsplit))){
    ii <- grep("AltName", recsplit)
    alt <- lapply(ii, function(kk) gsub("(AltName: )(.*)=(.*)", "\\3", recsplit[kk]) )
